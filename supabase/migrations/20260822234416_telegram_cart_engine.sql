...
create or replace function private.telegram_cart_change(
  p_store_id text,
  p_item_id text,
...
begin
  ...
  insert into public."StoreCustomer" (
    ...
    gen_random_uuid()::text, p_store_id, 'telegram', p_external_user_id,
...
create or replace function private.telegram_cart_snapshot(
  p_store_id text,
  p_external_user_id text
)
returns jsonb
...
create or replace function private.telegram_checkout_cart(
  p_store_id text,
  p_external_user_id text,
...
revoke all on function private.telegram_cart_change(text,text,text,text,text,integer) from public, anon, authenticated;
revoke all on function private.telegram_cart_snapshot(text,text) from public, anon, authenticated;
revoke all on function private.telegram_checkout_cart(text,text,text,text) from public, anon, authenticated;
grant execute on function private.telegram_cart_change(text,text,text,text,text,integer) to service_role;
grant execute on function private.telegram_cart_snapshot(text,text) to service_role;
grant execute on function private.telegram_checkout_cart(text,text,text,text) to service_role;