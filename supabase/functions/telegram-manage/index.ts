...
const authenticated = withSupabase({ auth: "user" }, async (request, ctx) => {
  ...
  try {
    if (action === "update_welcome") {
      ...
      const { data: last } = await admin.from("TelegramButton").select("sortOrder").eq("botId", bot.id).eq("parentId", parentId).order("sortOrder", { ascending: false }).limit(1);
      ...
      if (currentError) throw new Error(`button:${currentError.message}`);
      if (!current) return json({ ok: false, message: "دکمه پیدا نشد." }, 404);
      await admin.from("TelegramButton").update({ parentId: null }).eq("botId", bot.id).eq("parentId", buttonId);
      ...
      if (!ids.length || ids.length > 100) return json({ ok: false, message: "لیست ترتیب معتبر نیست." }, 400);
      const { data: owned, error: ownedError } = await admin.from("TelegramButton").select("id").eq("botId", bot.id).in("id", ids);
