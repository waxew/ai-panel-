...
// راهنما: این Type با نام «InstagramAccount» شکل و مقادیر مجاز داده را برای TypeScript مشخص می‌کند و در زمان اجرا کد مستقلی تولید نمی‌کند.
...
// راهنما: این Type با نام «InstagramRule» شکل و مقادیر مجاز داده را برای TypeScript مشخص می‌کند و در زمان اجرا کد مستقلی تولید نمی‌کند.
type InstagramRule = {
  id: string;
...
// راهنما: این Type با نام «InstagramEvent» شکل و مقادیر مجاز داده را برای TypeScript مشخص می‌کند و در زمان اجرا کد مستقلی تولید نمی‌کند.
type InstagramEvent = {
  id: string;
  eventType: string;
...
// راهنما: این دستور متغیر/ثابت «emptyDashboard» را تعریف می‌کند و مقدار موردنیاز این بخش از برنامه را نگه می‌دارد.
...
// راهنما: این تابع «requestDashboard» یک بخش مستقل از منطق برنامه را تعریف می‌کند؛ ورودی‌ها را می‌گیرد و منطق داخل بدنه را اجرا می‌کند.
async function requestDashboard(init?: RequestInit) {
  ...
  if (!response.ok) /* راهنما: این دستور عمداً یک خطا ایجاد می‌کند تا اجرای مسیر فعلی متوقف و کنترل خطا به لایه بالاتر منتقل شود. */ throw new Error(data.message || 'خطا در دریافت اطلاعات اینستاگرام');
...
// راهنما: این تابع «faNumber» یک بخش مستقل از منطق برنامه را تعریف می‌کند؛ ورودی‌ها را می‌گیرد و منطق داخل بدنه را اجرا می‌کند.
function faNumber(value: number | string | null | undefined) {
  // راهنما: این Return اجرای تابع را در این نقطه تمام می‌کند و «new Intl.NumberFormat('fa-IR').format(Number(value ?? 0))» را به فراخواننده برمی‌گرداند؛ در کامپوننت React می‌تواند UI خروجی باشد.
  return new Intl.NumberFormat('fa-IR').format(Number(value ?? 0));
}

// راهنما: این تابع «triggerLabel» یک بخش مستقل از منطق برنامه را تعریف می‌کند؛ ورودی‌ها را می‌گیرد و منطق داخل بدنه را اجرا می‌کند.
function triggerLabel(type: InstagramRule['triggerType']) {
  // راهنما: این شرط بررسی می‌کند آیا «type === 'COMMENT_KEYWORD'» برقرار است؛ فقط در صورت درست بودن، شاخه مربوط اجرا می‌شود.
  if (type === 'COMMENT_KEYWORD') /* راهنما: این Return اجرای تابع را در این نقطه تمام می‌کند و «'کامنت → دایرکت'» را به فراخواننده برمی‌گرداند؛ در کامپوننت React می‌تواند UI خروجی باشد. */ return 'کامنت → دایرکت';
  // راهنما: این شرط بررسی می‌کند آیا «type === 'DM_KEYWORD'» برقرار است؛ فقط در صورت درست بودن، شاخه مربوط اجرا می‌شود.
  if (type === 'DM_KEYWORD') /* راهنما: این Return اجرای تابع را در این نقطه تمام می‌کند و «'کلمه در دایرکت'» را به فراخواننده برمی‌گرداند؛ در کامپوننت React می‌تواند UI خروجی باشد. */ return 'کلمه در دایرکت';
  // راهنما: این Return اجرای تابع را در این نقطه تمام می‌کند و «'پاسخ به استوری'» را به فراخواننده برمی‌گرداند؛ در کامپوننت React می‌تواند UI خروجی باشد.
  return 'پاسخ به استوری';
}

// راهنما: این تابع «outcomeLabel» یک بخش مستقل از منطق برنامه را تعریف می‌کند؛ ورودی‌ها را می‌گیرد و منطق داخل بدنه را اجرا می‌کند.
function outcomeLabel(value: string) {
  ...
  // راهنما: این Return اجرای تابع را در این نقطه تمام می‌کند و «map[value] ?? value» را به فراخواننده برمی‌گرداند؛ در کامپوننت React می‌تواند UI خروجی باشد.
  return map[value] ?? value;
}

// راهنما: این تابع «InstagramControlCenter» یک بخش مستقل از منطق برنامه را تعریف می‌کند؛ ورودی‌ها را می‌گیرد و منطق داخل بدنه را اجرا می‌کند.
export default function InstagramControlCenter() {
  ...
  // راهنما: این دستور State محلی React برای «[saving, setSaving]» می‌سازد تا مقدار آن در UI نگهداری و با تغییرش صفحه دوباره Render شود.
  ...
  // راهنما: این دستور State محلی React برای «[accountId, setAccountId]» می‌سازد تا مقدار آن در UI نگهداری و با تغییرش صفحه دوباره Render شود.
  const [accountId, setAccountId] = useState('');
  // راهنما: این دستور State محلی React برای «[name, setName]» می‌سازد تا مقدار آن در UI نگهداری و با تغییرش صفحه دوباره Render شود.
  const [name, setName] = useState('');
  // راهنما: این دستور متغیر/ثابت «[triggerType, setTriggerType]» را تعریف می‌کند و مقدار موردنیاز این بخش از برنامه را نگه می‌دارد.
  const [triggerType, setTriggerType] = useState<InstagramRule['triggerType']>('COMMENT_KEYWORD');
  // راهنما: این دستور State محلی React برای «[keywords, setKeywords]» می‌سازد تا مقدار آن در UI نگهداری و با تغییرش صفحه دوباره Render شود.
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setData(result)».
      setData(result);
      // راهنما: این شرط بررسی می‌کند آیا «!accountId && result.accounts[0]?.id» برقرار است؛ فقط در صورت درست بودن، شاخه مربوط اجرا می‌شود.
      if (!accountId && result.accounts[0]?.id) /* راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setAccountId(result.accounts[0].id)». */ setAccountId(result.accounts[0].id);
    } catch (err) {
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setError(err instanceof Error ? err.message : 'اطلاعات اینستاگرام دریافت نشد.')».
  ...
  // راهنما: این دستور مقدار «selectedAccount» را با useMemo محاسبه و Cache می‌کند تا محاسبه غیرضروری در Renderهای بعدی تکرار نشود.
  const selectedAccount = useMemo(() => data.accounts.find((item) => item.id === accountId) ?? null, [data.accounts, accountId]);
  // راهنما: این دستور متغیر/ثابت «connectionReady» را تعریف می‌کند و مقدار موردنیاز این بخش از برنامه را نگه می‌دارد.
  ...
  // راهنما: این تابع «submitRule» یک بخش مستقل از منطق برنامه را تعریف می‌کند؛ ورودی‌ها را می‌گیرد و منطق داخل بدنه را اجرا می‌کند.
  async function submitRule(event: FormEvent) {
    // راهنما: این دستور یک عملیات اجرایی انجام می‌دهد: «event.preventDefault()».
    event.preventDefault();
    // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setSaving(true)».
    setSaving(true);
    // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setError('')».
    setError('');
    ...
    try {
      ...
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setData(result)».
      setData(result);
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setName('')».
      setName('');
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setNotice(result.activationDeferred ? 'Rule ذخیره شد؛ بعد از اتصال Meta می‌توان آن را فعا…».
      setNotice(result.activationDeferred ? 'Rule ذخیره شد؛ بعد از اتصال Meta می‌توان آن را فعال کرد.' : 'Rule ذخیره شد.');
    } catch (err) {
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setError(err instanceof Error ? err.message : 'ذخیره Rule انجام نشد.')».
      setError(err instanceof Error ? err.message : 'ذخیره Rule انجام نشد.');
    } finally {
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setSaving(false)».
      setSaving(false);
    }
  }
  ...
  async function toggleRule(rule: InstagramRule) {
    ...
    try {
      ...
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setError(err instanceof Error ? err.message : 'تغییر وضعیت Rule انجام نشد.')».
      setError(err instanceof Error ? err.message : 'تغییر وضعیت Rule انجام نشد.');
    }
  }

  // راهنما: این تابع «deleteRule» یک بخش مستقل از منطق برنامه را تعریف می‌کند؛ ورودی‌ها را می‌گیرد و منطق داخل بدنه را اجرا می‌کند.
  async function deleteRule(ruleId: string) {
    ...
    try {
      ...
      // راهنما: این دستور با فراخوانی Setter، State مربوط را تغییر می‌دهد تا داده جدید در UI اعمال شود: «setError(err instanceof Error ? err.message : 'حذف Rule انجام نشد.')».
      setError(err instanceof Error ? err.message : 'حذف Rule انجام نشد.');
    }
  }

  // راهنما: این Return اجرای تابع را در این نقطه تمام می‌کند و «( <div className="ig-page" dir="rtl"> <style>{styles}</style> <header clas…» را به فراخواننده برمی‌گرداند؛ در کامپوننت React می‌تواند UI خروجی باشد.
  return (
