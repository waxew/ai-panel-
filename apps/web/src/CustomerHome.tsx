...
// راهنما: این تابع «CustomerHome» یک بخش مستقل از منطق برنامه را تعریف می‌کند؛ ورودی‌ها را می‌گیرد و منطق داخل بدنه را اجرا می‌کند.
export default function CustomerHome() {
  ...
  useEffect(() => {
    ...
    // راهنما: این دستور یک عملیات اجرایی انجام می‌دهد: «void fetch('/api/customer/dashboard', { // از سرور صریحاً پاسخ JSON درخواست می‌کنیم. head…».
    void fetch('/api/customer/dashboard', {
      ...
      .then(async (response) => {
        ...
        if (!response.ok) /* راهنما: این دستور عمداً یک خطا ایجاد می‌کند تا اجرای مسیر فعلی متوقف و کنترل خطا به لایه بالاتر منتقل شود. */ throw new Error(payload.message || 'dashboard_failed');
  ...
  // راهنما: این Return اجرای تابع را در این نقطه تمام می‌کند و «( // div ریشه صفحه است؛ کلاس simple-home برای استایل و dir=rtl برای راست‌ب…» را به فراخواننده برمی‌گرداند؛ در کامپوننت React می‌تواند UI خروجی باشد.
  return (
