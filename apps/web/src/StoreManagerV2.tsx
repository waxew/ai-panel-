...
export default function StoreManagerV2() {
  ...
  async function bulkSetPrice(event: FormEvent) {
    ...
    if (!Number.isSafeInteger(price) || price < 0) { setMessage('قیمت جدید معتبر نیست.'); setMessageOk(false); return; }
    ...
    if (!window.confirm(`قیمت ${target} روی ${money(price, data?.store?.currency ?? 'IRR')} تنظیم شود؟`)) return;
  ...
  async function deleteCategory(category: Category) {
    ...
    if (await action({ action: 'delete_category', categoryId: category.id }, 'دسته‌بندی حذف شد.')) {
      ...
      if (bulkCategoryId === category.id) setBulkCategoryId('');
    }
  }
