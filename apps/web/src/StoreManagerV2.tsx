...
export default function StoreManagerV2() {
  ...
  const [typeDelivery, setTypeDelivery] = useState<'DIGITAL' | 'PHYSICAL' | 'SERVICE'>('DIGITAL');
  const [showCategories, setShowCategories] = useState(false);
  const [showTypes, setShowTypes] = useState(false);
  const [product, setProduct] = useState({ title: '', description: '', priceAmount: '', inventoryCount: '', productTypeId: '', categoryId: '' });
  ...
  const load = useCallback(async () => {
    ...
    setProduct((current) => ({
      ...current,
      productTypeId: current.productTypeId || next.productTypes?.[0]?.id || '',
    }));
  }, []);
  ...
  async function createProduct(event: FormEvent) {
    ...
    if (ok) setProduct((current) => ({ title: '', description: '', priceAmount: '', inventoryCount: '', productTypeId: current.productTypeId, categoryId: current.categoryId }));
  }
  ...
  async function deleteProductType(productType: ProductType) {
    if (!window.confirm(`نوع «${productType.title}» حذف شود؟ محصولات قبلی حذف نمی‌شوند.`)) return;
  ...
  return <div className="store-simple" dir="rtl"><style>{styles}</style><main>
    <header className="top"><div><a className="back" href="/app">← داشبورد</a><h1>{data.store.name}</h1><p>محصولات، نوع‌ها و دسته‌ها را بدون تنظیمات فنی مدیریت کن.</p></div><a className="orders" href="/app/orders">سفارش‌ها</a></header>
    ...
    <section className="products card">
      ...
      {data.items.length === 0 ? <div className="empty">هنوز محصولی ثبت نشده.</div> : <div className="product-list">{data.items.map((item) => { const customType = item.metadata?.productTypeId ? typeMap.get(item.metadata.productTypeId) : undefined; const category = item.categoryId ? categoryMap.get(item.categoryId) : undefined; return <div key={item.id}><span><b>{item.title}</b><small>{customType?.title || deliveryLabels[item.itemType] || 'محصول'}{category ? ` · ${category.title}` : ''}</small></span><strong>{money(item.priceAmount, item.currency)}</strong></div>; })}</div>}
    </section>
  </main></div>;
}

const styles = `
.store-simple{min-height:100vh;background:#080b10;color:#f4f6fa;font-family:Inter,Vazirmatn,system-ui,-apple-system,sans-serif}.store-simple *{box-sizing:border-box}.store-simple main{max-width:1180px;margin:auto;padding:34px 20px 110px}.store-simple .narrow{max-width:720px}.back{color:#8290a4;text-decoration:none;font-size:12px}.top{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:22px}.top h1,.hero h1{font-size:32px;margin:8px 0 6px}.top p,.hero p{color:#8592a5;margin:0;line-height:1.8}.orders,.primary,.store-simple button{border:0;border-radius:11px;padding:11px 14px;font:800 12px inherit;cursor:pointer}.orders,.primary{background:#f4f6fa;color:#080b10;text-decoration:none}.store-simple button:disabled{opacity:.5;cursor:wait}.card{border:1px solid #232c3b;background:#0d131c;border-radius:17px}.notice{margin:0 0 14px;padding:12px 14px;border:1px solid #60323b;background:#25151a;color:#ffc1cb;border-radius:12px;font-size:12px}.notice.ok{border-color:#245b46;background:#0d241b;color:#9de5c7}.summary{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px}.summary article{border:1px solid #222c3a;background:#0d131c;border-radius:14px;padding:15px}.summary b{font-size:24px;display:block}.summary span{font-size:11px;color:#7f8c9f}.layout{display:grid;grid-template-columns:1.3fr .7fr;gap:12px}.add-product{padding:18px}.section-title{display:flex;justify-content:space-between;align-items:center;margin-bottom:13px}.section-title span{font-size:10px;color:#7f8c9f}.section-title h2{margin:4px 0;font-size:20px}.add-product form{display:grid;grid-template-columns:1fr 1fr;gap:10px}.add-product label{display:grid;gap:6px;color:#a9b3c1;font-size:11px}.add-product label small{color:#687588}.add-product input,.add-product select,.add-product textarea,.inline-add input,.inline-add select{width:100%;border:1px solid #293447;background:#080d14;color:#fff;border-radius:10px;padding:11px;outline:none}.wide{grid-column:1/-1}.side{display:grid;align-content:start;gap:12px}.manager{overflow:hidden}.manager-head{width:100%;background:transparent;color:#fff;display:flex;align-items:center;justify-content:space-between;text-align:right;padding:16px}.manager-head span{display:grid;gap:4px}.manager-head small{font-size:9px;color:#738095}.manager-head em{font-style:normal;color:#9eabba;font-size:10px}.manager-body{padding:0 14px 14px;border-top:1px solid #202938}.inline-add{display:grid;grid-template-columns:1fr 105px auto;gap:6px;padding:13px 0}.inline-add.category{grid-template-columns:1fr auto}.inline-add button,.rows button{background:#17202d;color:#d9e0ea}.rows{display:grid}.rows>div{display:grid;grid-template-columns:1fr auto auto;gap:6px;align-items:center;padding:9px 0;border-top:1px solid #1d2633}.rows span{display:grid;gap:3px}.rows b{font-size:11px}.rows small{font-size:9px;color:#758297}.rows .danger{color:#ffb4c0}.products{margin-top:12px;padding:18px}.product-list{display:grid}.product-list>div{display:flex;justify-content:space-between;align-items:center;gap:14px;padding:12px 2px;border-top:1px solid #1d2633}.product-list span{display:grid;gap:4px}.product-list b,.product-list strong{font-size:12px}.product-list small{font-size:10px;color:#778499}.empty{padding:28px;text-align:center;color:#6d7a8d;border:1px dashed #2a3444;border-radius:12px}.setup{padding:20px;margin-top:24px;display:grid;gap:12px}.setup label{display:grid;gap:7px;color:#9faaba;font-size:11px}.setup input{border:1px solid #293447;background:#080d14;color:#fff;border-radius:10px;padding:12px}.setup button{background:#f4f6fa;color:#080b10}.loading{display:grid;place-items:center;color:#8290a4}
@media(max-width:820px){.layout{grid-template-columns:1fr}.summary{grid-template-columns:repeat(3,1fr)}}@media(max-width:560px){.store-simple main{padding:24px 13px 90px}.top{display:grid}.summary{grid-template-columns:repeat(3,1fr)}.summary article{padding:11px}.summary b{font-size:19px}.add-product form{grid-template-columns:1fr}.wide{grid-column:auto}.inline-add,.inline-add.category{grid-template-columns:1fr}.rows>div{grid-template-columns:1fr auto auto}.product-list>div{align-items:flex-start;flex-direction:column}.top h1,.hero h1{font-size:27px}}
`;
