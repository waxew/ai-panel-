...
const links = [
  ...
  ['/app/booking/feedback', 'رضایت‌سنجی'],
...
export default function CommerceQuickNav() {
  ...
  return <nav className="commerce-quick-nav" dir="rtl" aria-label="Commerce navigation"><style>{styles}</style>{links.map(([href,label])=><a key={href} href={href} className={path===href?'active':''}>{label}</a>)}{path==='/app/instagram'&&<button className="meta-connect" type="button" disabled={connecting} onClick={()=>void connectInstagram()}>{connecting?'در حال اتصال…':'اتصال Meta'}</button>}</nav>;
}
...
.commerce-quick-nav a{color:#8f9bad;text-decoration:none}.commerce-quick-nav a:hover,.commerce-quick-nav a.active{background:#eef3f9;color:#090e15}
.commerce-quick-nav .meta-connect{border:1px solid #a855f7;background:linear-gradient(135deg,#7c3aed,#db2777);color:#fff;cursor:pointer}.commerce-quick-nav .meta-connect:disabled{opacity:.6;cursor:wait}
