...
async function request(body?:Record<string,unknown>){
  ...
  if(!res.ok) throw new Error(data.message||'خطا در نوبت‌دهی');
  return data;
}
...
export default function BookingManagerV2(){
  ...
  async function act(body:Record<string,unknown>){
    ...
    try{setData(await request(body));return true;}catch(e){setError(e instanceof Error?e.message:'عملیات انجام نشد');return false;}finally{setBusy(false);}
  }
  ...
  async function createAppointment(e:FormEvent){
    ...
    if(await act({action:'create_appointment',...booking,startsAt})){
      setShowNew(false);setBooking(v=>({...v,customerName:'',phone:'',note:''}));setTab('appointments');
    }
  }
  async function createService(e:FormEvent){e.preventDefault();if(await act({action:'create_service',title:service.title,durationMinutes:Number(service.durationMinutes),priceAmount:Number(service.priceAmount)})){setShowService(false);setService({title:'',durationMinutes:'60',priceAmount:'0'});}}
  async function createStaff(e:FormEvent){e.preventDefault();if(await act({action:'create_staff',...staffForm})){setShowStaff(false);setStaffForm({name:'',roleTitle:''});}}
  async function status(id:string,status:Appointment['status']){await act({action:'update_appointment_status',id,status});}
