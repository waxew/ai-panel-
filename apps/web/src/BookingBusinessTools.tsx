...
async function loadDashboard(){
  ...
  if(!res.ok) throw new Error(data.message || 'اطلاعات ابزارهای نوبت‌دهی دریافت نشد.');
  return data;
}
...
export default function BookingBusinessTools(){
  ...
  function metricFor(key:ToolKey){
    if(!data) return '—';
    if(key==='customers') return nf.format(data.customers.length);
    if(key==='booking') return nf.format(data.appointments.length);
    if(key==='staff-access') return nf.format(data.staff.length);
    if(key==='staff-services') return nf.format(data.services.length);
    if(key==='winback') return nf.format(stats.returning);
    if(key==='finance') return money(stats.revenue);
    if(key==='reports') return nf.format(stats.thisMonth);
    if(key==='online-booking') return data.settings?.publicBookingEnabled ? 'فعال' : 'خاموش';
    return undefined;
  }
...
function Icon({tool}:{tool:Tool}){return <i className="bt-icon" style={{color:tool.color,boxShadow:`inset 0 0 0 1px ${tool.color}55,0 0 22px ${tool.color}18`}}>{tool.icon}</i>}
function Crown(){return <span className="bt-crown">♛</span>}
...
function ToolDrawer({tool,data,stats,close}:{tool:Tool;data:Dashboard|null;stats:{revenue:number;returning:number;thisMonth:number};close:()=>void}){
  ...
  const detail = (()=>{
    switch(tool.key){
      case 'finance': return `درآمد فعلی ثبت‌شده از نوبت‌های تأییدشده و انجام‌شده: ${money(stats.revenue)}`;
      case 'reports': return `${nf.format(stats.thisMonth)} نوبت در ماه جاری در دیتابیس ثبت شده است.`;
      case 'customers': return `${nf.format(data?.customers.length||0)} پرونده مشتری در CRM موجود است.`;
      case 'booking': return `${nf.format(data?.appointments.length||0)} نوبت در سیستم ثبت شده است.`;
      case 'staff-access': return `${nf.format(data?.staff.length||0)} پرسنل فعال/ثبت‌شده برای تعریف سطح دسترسی داریم.`;
      case 'staff-services': return `${nf.format(data?.staff.length||0)} پرسنل و ${nf.format(data?.services.length||0)} خدمت ثبت شده است.`;
      case 'online-booking': return data?.settings?.publicBookingEnabled ? 'رزرو آنلاین در تنظیمات فعال است؛ مرحله بعد صفحه عمومی مشتری است.' : 'رزرو آنلاین فعلاً خاموش است.';
      case 'sms': return 'هسته پنل پیامک در UI آماده است؛ برای ارسال واقعی باید سرویس‌دهنده پیامک و API Key متصل شود.';
      case 'birthday': return 'برای فعال‌سازی کامل، تاریخ تولد باید به پرونده مشتری اضافه و یک زمان‌بند ارسال ساخته شود.';
      case 'survey': return 'مرحله بعد: ساخت لینک نظرسنجی و ذخیره امتیاز/نظر هر مشتری بعد از مراجعه.';
      case 'reminder': return 'مرحله بعد: تعریف فاصله مراجعه برای هر خدمت و ساخت یادآوری زمان‌بندی‌شده.';
      case 'winback': return `${nf.format(stats.returning)} مشتری در داده فعلی بیش از یک نوبت دارند؛ منطق بازگشت مشتری بر اساس آخرین مراجعه اضافه می‌شود.`;
      default: return 'این ماژول در هاب محصول قرار گرفت و در فاز اتصال بک‌اند اختصاصی تکمیل می‌شود.';
    }
  })();

  return <div className="bt-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)close();}}><section className="bt-drawer">
